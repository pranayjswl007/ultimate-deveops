...
def update_existing_promotion_pr(repo, pr_number, source_pr, gh_pat):
    ...
    if response.status_code != 200:
        print(f"⚠️ Warning: Could not fetch existing PR details: {response.status_code}")
        return
...
"""
    ...
    if response.status_code == 200:
        print(f"✅ Updated existing promotion PR #{pr_number} with new source PR reference")
    else:
        print(f"⚠️ Warning: Could not update existing PR body: {response.status_code}")
...
def create_promotion_pr(repo, head, base, source_pr, gh_pat, has_conflict):
    ...
    if existing_pr_number:
        print(f"📋 Promotion PR already exists: #{existing_pr_number}")
...
"""
    ...
    if response.status_code == 422:
        ...
        if "pull request already exists" in error_data.get("message", "").lower():
            print("🔄 PR was created by another process, searching for it...")
            existing_pr_number = find_existing_promotion_pr(repo, head, base, gh_pat)
            if existing_pr_number:
                print(f"📋 Found the existing PR: #{existing_pr_number}")
                update_existing_promotion_pr(repo, existing_pr_number, source_pr, gh_pat)
